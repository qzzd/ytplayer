...
export class VideosSearchComponent {
  ...
  doSearch(event): void {
    if (this.loadingInProgress ||
      ...
      (this.lastSearch && this.lastSearch === this.searchForm.value.query)) {
      return;
    }

    this.videosUpdated.emit([]);
    this.lastSearch = this.searchForm.value.query;

    this.youtubeService.searchVideos(this.lastSearch)
      .then(data => {
        if (data.length < 1) {
